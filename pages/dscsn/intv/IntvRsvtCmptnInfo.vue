...
const selectedInfo: Ref<DscsnRsvtDtInVO> = ref({
  rsvtNo: route.query.rsvtNo //TODO 예약번호 이전 화면에서 값을 받아와서 처리되도록 수정 필요
});
const selectDscsnRsvtDt = async () => {
  const payload = selectedInfo.value;
  const response = await dscsnApis.selectDscsnRsvtDt(payload);
  form.rsvtNo = response.data.rsvtNo;
  form.dscsnRsvtDt = response.data.dscsnRsvtDt;
  form.brofNm = response.data.brofNm;
  form.rcptDt = response.data.rcptDt;
  form.dscsnCn = response.data.dscsnCn;
  form.brofAddr = response.data.brofAddr;
  form.brofDaddr = response.data.brofDaddr;

  //지사가 없을 경우
  if (form.brofAddr === null) {
    form.brofDispAddr = form.brofNm;
  } else {
    form.brofDispAddr =
      form.brofNm + ' / ' + form.brofAddr + ' ' + form.brofDaddr;
  }

  form.brofRprsTelno = response.data.brofRprsTelno;
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <span class="is-emphasis">{{ form.dscsnRsvtDt }}</span>
                </td>
              </tr>
              <tr>
...
@use 'assets/style/pages/dscsn/intv/IntvRsvtCmptnInfo';
</style>
