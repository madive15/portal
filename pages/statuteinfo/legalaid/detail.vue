...
import { useRlfStore } from '~/stores/rlf';
...
const rlfCase = useRlfStore();
const { rlfCaseDetail } = storeToRefs(rlfCase);
...
onBeforeMount(async () => {
  await rlfCase.searchCase(route.query);
});
...
const kakao = async () => {
  try {
    ...
    window.Kakao.Link.sendDefault({
      ...
      content: {
        title: '법률구조사례',
        description: rlfCaseDetail.value.caseTtl,
        imageUrl: '',
        link: {
...
watch(rlfCaseDetail, async (newValue) => {
        ...
        <div class="container">
          <div class="board-view-box">
            <div class="content-header">
              <span class="category-span" v-if="!!rlfCaseDetail.lwaCtgryNm">{{
                rlfCaseDetail.lwaCtgryNm
              }}</span>
              <div class="info-box">
                <div class="title-box">
                  <CommonTitle tag="h4" size="1">{{
                    rlfCaseDetail.caseTtl
                  }}</CommonTitle>
                  <div class="share-box">
                ...
                <div class="inner-box">
                  <span class="date-span">
                    작성일: {{ rlfCaseDetail.frstRegDt.substring(0, 10) }}</span
                  >
                  <span class="hit-span"
                    >조회수:
                    {{ rlfCaseDetail.inqCnt }}
                  </span>
                </div>
              </div>
            </div>
            <div class="content-body">
              <div class="answer-box">
                <div v-html="rlfCaseDetail.caseCn"></div>
              </div>
            </div>
            <div v-if="rlfCaseDetail.atchFileId != null" class="content-footer">
              <div class="file-box">
              ...
              <div class="file-info-box">
                <button
                  ...
                  @click="() => download(rlfCaseDetail.atchFileId)"
                >
                ...
                <button
                  ...
                  v-if="rlfCaseDetail.pdfId == null"
                >
                  <i class="icon-shortcut"></i>
                  <NuxtLink
                    :href="`/streamdocs/view/sd;streamdocsId=${rlfCaseDetail.pdfId}`"
                    target="_blank"
                    class="btn-basic primary sm"
                    title="새 창 열림"
                  >
...
@use '~/assets/style/pages/statuteinfo/legalaid/detail.scss';
</style>
