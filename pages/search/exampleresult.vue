...
const { caseList, caseListQuery, caseGroupList } = storeToRefs(caseStore);

onMounted(async () => {
  if (caseGroupList.value.length > 0) {
    setDetailOptions();
  }
});
...
const changeSize = (e) => {
  caseListQuery.value.size = e.value; // 접근성 Dropdown 수정 25.04.08
  caseListQuery.value.defaultText = e.text; // 접근성 Dropdown 추가 25.04.08
  caseListQuery.value.page = 1;
  reSearch();
};

const activeButton = ref(null);
const desc = (index) => {
  caseListQuery.value.sort = index;
  caseListQuery.value.page = 1;
  reSearch();
...
// 사례 구분 체크 박스
const caseClsfCheckboxes = ref([]);

const setDetailOptions = () => {
  setInstCheckboxes();
  setCaseClsfCheckboxes();
};

// 기관 체크박스
const setInstCheckboxes = () => {
  if (caseGroupList.value && caseGroupList.value.length > 0) {
    instCheckboxes.value = caseGroupList.value[0].labels;
  } else {
...
// 사례 구분 체크박스
const setCaseClsfCheckboxes = () => {
  if (caseGroupList.value && caseGroupList.value.length > 0) {
    caseClsfCheckboxes.value = caseGroupList.value[1].labels;
  } else {
    caseClsfCheckboxes.value = [];
  }
};

// 첫 번째 그룹의 "전체" 체크박스 상태 관리
const caseClsfSelectAll = computed<boolean>({
  get() {
    return caseClsfCheckboxes.value.every((checkbox) => !checkbox.checked);
  },
  set(value: boolean) {
    if (value) {
      caseClsfCheckboxes.value.forEach((checkbox) => {
...
const updateToolTipState = () => {
  const isCaseClsfNotDefault = !caseClsfSelectAll.value;
  const hasAdditionalDetailItems = detailItems.value.some(
    (item) => item !== '전체'
  );

  showToolTip.value = isCaseClsfNotDefault || hasAdditionalDetailItems;
};
...
const removeDetailItem = (index: number) => {
  ...
  if (item.startsWith('기관: ')) {
    ...
    // 사례 구분 체크박스의 checked 상태를 false로 설정
    const label = item.replace('사례 구분: ', '');
    const checkbox = [...caseClsfCheckboxes.value].find(
      (cb) => processCaseClsfLabel(cb.label) === label
    );
    if (checkbox) {
      checkbox.checked = false;
    }
...
// 탭 이동 시 검색 결과 초기화 함수
const resetCaseState = (isEqual: boolean) => {
  resetInstCheckboxes();
  resetCaseClsfCheckboxes();
  hideDetail();
  caseListQuery.value.size = 20;
  caseListQuery.value.sort = 'A';
  caseListQuery.value.defaultText = '20개씩 보기'; // 접근성 Dropdown 수정 25.04.08
...
const search = () => {
  ...
  if (isInstChecked) {
    ...
    caseListQuery.value.instNo = [...checkedInst].join(' ');
  } else {
    caseListQuery.value.instNo = '';
  }

  // 사례 구분 체크박스 중 선택된 것이 있는지 확인
  const isCaseClsfChecked = caseClsfCheckboxes.value.some(
    (checkbox) => checkbox.checked
  );

  // 사례 구분 검색
  if (caseClsfSelectAll.value) {
    caseListQuery.value.caseClsfCd = '';
  } else if (isCaseClsfChecked) {
    const checkedCaseClsf = caseClsfCheckboxes.value
      .filter((checkbox) => checkbox.checked)
      .map((checkbox) => checkbox.id);
    caseListQuery.value.caseClsfCd = [...checkedCaseClsf].join(' ');
  } else {
    // 체크된 항목이 없으면 경고 메시지 표시
    showAlert({
      icon: 'warning',
      text: '하나의 사례 구분을 체크하셔야 검색이 가능합니다.'
    });
    return;
  }

  // 결과 내 재검색어
  caseListQuery.value.reSearchQuery = reSearchQueryList.value;
...
const updateDetailItems = () => {
  ...
  // 사례 구분에서 선택된 항목 처리
  const selectedCaseClsfLabel = [];
  if (caseClsfSelectAll.value) {
    selectedCaseClsfLabel.push('전체');
  } else {
    const selectedCaseClsf = caseClsfCheckboxes.value
      .filter((checkbox) => checkbox.checked)
      .map((checkbox) => processCaseClsfLabel('사례 구분: ' + checkbox.label));
    selectedCaseClsfLabel.push(...selectedCaseClsf);
  }
  ...
  detailItems.value = [
    ...selectedInst,
    ...selectedCaseClsfLabel,
    ...reSearchQueryLabel
  ];
};

// 서비스 분류 라벨 처리 함수
const processCaseClsfLabel = (label: string): string => {
  // 라벨에 '/'가 있는 경우 ' > '로 대체
          ...
          <transition name="fade">
            <div class="detail-table-wrap">
              <div class="result-table-box detail-table-box">
                <table>
                  <caption>
                    사례 항목의 기관, 사례 구분, 결과 내 재검색, 선택한 검색
                    조건 옵션 선택 표
                  </caption>
                  <colgroup>
                    <col style="width: 14%" />
                      ...
                      <tr>
                        ...
                        <td class="check-td">
                          <div class="check-box">
                            ...
                            <div
                              class="form-check check-input-box"
                              v-for="(checkbox, index) in caseClsfCheckboxes"
                              :key="checkbox.id"
                            >
                              <input
                                class="form-check-input check-input"
            ...
            <div class="service-box">
              <div class="search-info-wrap">
                <div class="list-search-box">
                  <strong>
                    총 <span>{{ caseCnt }}건</span>의 관련 사례가 있습니다.
                  </strong>
                </div>
                <div class="view-box">
                  ...
                  <!-- 접근성 Dropdown 추가 25.04.08 -->
                  <Dropdown
                    :buttonText="caseListQuery.defaultText"
                    :drop-slots="true"
                    class="select-box"
                  >
                    <li
                    ...
                    >
                      <button
                        ...
                        :title="
                          caseListQuery.size === item.value ? '선택됨' : ''
                        "
                      >
                  ...
                  <template v-for="(cases, index) in caseList.content">
                    ...
                    <div class="col-xl-10 col-12 a-col">
                      <div class="list-text-box">
                        <NuxtLink
                          :to="`/statuteinfo/discussion/detail?caseNo=${cases.caseNo}&ctgryNo=${cases.instNo}`"
                          class="link-txt"
                          :external="true"
                        >
                          <dl>
                            <dt v-html="cases.caseTtl"></dt>
                            <dd class="title-dd" v-html="cases.caseCn"></dd>
                            <dd class="etc-dd">
                              <span class="date-span">
                                작성일 : {{ formatDate(cases.regDt) }}
                              </span>
                              <span class="vr"></span>
                              <span class="hit-span">
                                조회수 : {{ cases.inqCnt }}
                    ...
                    <div class="col-xl-2 col-12 cate-col">
                      <span class="bedge-span">{{ cases.caseClsfNm }}</span>
                    </div>
                  --></template>
                </div>
              </div>
...
@use '~/assets/style/pages/search/exampleresult.scss';
</style>
