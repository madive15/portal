...
/*
 * 화면명 : 전자접수안내, 전자접수이용절차
 * 화면ID : LASP_ELE_001, LASP_ELE_002
 * 파일명 : SlfdgnsInfo
 * 작성자 : 문경훈
...
/**
 * name : goToElrcpt
 * desc : 전자접수 신청 화면으로 이동
 **/
const goToElrcpt = () => {
  // 로그인 완료상태
  if (authenticated.value) {
    navigateTo('/elrcpt/slfdgns/SlfdgnsEdit');
  } else {
    showAlert({
      icon: 'info',
      ...
      .then((result) => {
        if (result.isConfirmed) {
          navigateTo({
            ...
            query: { redirect: '/elrcpt/slfdgns/SlfdgnsEdit' } // 전자접수 자가진단화면으로 이동
          });
        }
      })
      .catch(() => {});
      ...
      <div class="container">
        ...
        >
          <CommonTabItem title="전자접수 안내">
            ...
            <div class="bull-list-box">
              <BullList type="number" number-style="period">
                <li class="!block">
            ...
            <ButtonGroup align="center" class="!mt-50px">
              <CommonButton variants="primary" width="sm-1" @click="goToElrcpt">
                전자접수 신청
              </CommonButton>
            </ButtonGroup>
          </CommonTabItem>
...
@use '~/assets/style/pages/elrcpt/slfdgns/SlfdgnsInfo.scss';
</style>
